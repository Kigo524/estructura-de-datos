...
// Agregar una conexión (arista) en un grafo dirigido
...
void bfs(Grafo* grafo, int inicio) {
    ...
    int* cola = (int*) malloc(grafo->numVertices * sizeof(int)); // Cola dinámica
    int frente = 0, fin = 0;
...
int main() {
    ...
    for (int i = 0; i < miGrafo->numVertices; i++) {
        Nodo* temp = miGrafo->array[i].cabeza;
        while (temp != NULL) {
